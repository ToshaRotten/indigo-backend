...
func (osq *OrderStatusQuery) QueryStatus() *OrderQuery {
...
func (osq *OrderStatusQuery) Clone() *OrderStatusQuery {
	...
	return &OrderStatusQuery{
		...
		withStatus: osq.withStatus.Clone(),
...
func (osq *OrderStatusQuery) WithStatus(opts ...func(*OrderQuery)) *OrderStatusQuery {
	query := (&OrderClient{config: osq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	osq.withStatus = query
	return osq
}
...
func (osq *OrderStatusQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*OrderStatus, error) {
	...
	if query := osq.withStatus; query != nil {
		if err := osq.loadStatus(ctx, query, nodes,
			func(n *OrderStatus) { n.Edges.Status = []*Order{} },
			func(n *OrderStatus, e *Order) { n.Edges.Status = append(n.Edges.Status, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (osq *OrderStatusQuery) loadStatus(ctx context.Context, query *OrderQuery, nodes []*OrderStatus, init func(*OrderStatus), assign func(*OrderStatus, *Order)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(order.FieldOrderStatusID)
	}
	query.Where(predicate.Order(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(orderstatus.StatusColumn), fks...))
	}))
