...
func (pcq *ProductCategoryQuery) QueryCategory() *ProductQuery {
...
func (pcq *ProductCategoryQuery) Clone() *ProductCategoryQuery {
	...
	return &ProductCategoryQuery{
		...
		withCategory: pcq.withCategory.Clone(),
...
func (pcq *ProductCategoryQuery) WithCategory(opts ...func(*ProductQuery)) *ProductCategoryQuery {
	query := (&ProductClient{config: pcq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	pcq.withCategory = query
	return pcq
}
...
func (pcq *ProductCategoryQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*ProductCategory, error) {
	...
	if query := pcq.withCategory; query != nil {
		if err := pcq.loadCategory(ctx, query, nodes,
			func(n *ProductCategory) { n.Edges.Category = []*Product{} },
			func(n *ProductCategory, e *Product) { n.Edges.Category = append(n.Edges.Category, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (pcq *ProductCategoryQuery) loadCategory(ctx context.Context, query *ProductQuery, nodes []*ProductCategory, init func(*ProductCategory), assign func(*ProductCategory, *Product)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(product.FieldProductCategoryID)
	}
	query.Where(predicate.Product(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(productcategory.CategoryColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "product_category_id" returned %v for node %v`, fk, n.ID)
		}
		assign(node, n)
	}
	return nil
