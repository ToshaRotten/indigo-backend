...
func (pcc *ProductCategoryCreate) AddCategoryIDs(ids ...int) *ProductCategoryCreate {
	pcc.mutation.AddCategoryIDs(ids...)
	return pcc
}

// AddCategory adds the "category" edges to the Product entity.
func (pcc *ProductCategoryCreate) AddCategory(p ...*Product) *ProductCategoryCreate {
	ids := make([]int, len(p))
	for i := range p {
		ids[i] = p[i].ID
	}
	return pcc.AddCategoryIDs(ids...)
}
...
func (pcc *ProductCategoryCreate) createSpec() (*ProductCategory, *sqlgraph.CreateSpec) {
	...
	if nodes := pcc.mutation.CategoryIDs(); len(nodes) > 0 {
