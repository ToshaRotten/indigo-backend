...
func (aq *AddressQuery) QueryAddress() *OrderQuery {
...
func (aq *AddressQuery) WithAddress(opts ...func(*OrderQuery)) *AddressQuery {
	query := (&OrderClient{config: aq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (aq *AddressQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Address, error) {
	...
	if query := aq.withUser; query != nil {
		if err := aq.loadUser(ctx, query, nodes,
			func(n *Address) { n.Edges.User = []*User{} },
			func(n *Address, e *User) { n.Edges.User = append(n.Edges.User, e) }); err != nil {
			return nil, err
		}
	}
	if query := aq.withAddress; query != nil {
		if err := aq.loadAddress(ctx, query, nodes,
			func(n *Address) { n.Edges.Address = []*Order{} },
			func(n *Address, e *Order) { n.Edges.Address = append(n.Edges.Address, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (aq *AddressQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*Address, init func(*Address), assign func(*Address, *User)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(address.UserTable)
		s.Join(joinT).On(s.C(user.FieldID), joinT.C(address.UserPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(address.UserPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(address.UserPrimaryKey[1]))
...
func (aq *AddressQuery) loadAddress(ctx context.Context, query *OrderQuery, nodes []*Address, init func(*Address), assign func(*Address, *Order)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(order.FieldAddressID)
	}
	query.Where(predicate.Order(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(address.AddressColumn), fks...))
	}))
