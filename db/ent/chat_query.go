...
func (cq *ChatQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Chat, error) {
	...
	if query := cq.withUser; query != nil {
		if err := cq.loadUser(ctx, query, nodes, nil,
			func(n *Chat, e *User) { n.Edges.User = e }); err != nil {
			return nil, err
		}
	}
	if query := cq.withChatMessage; query != nil {
		if err := cq.loadChatMessage(ctx, query, nodes,
			func(n *Chat) { n.Edges.ChatMessage = []*Message{} },
			func(n *Chat, e *Message) { n.Edges.ChatMessage = append(n.Edges.ChatMessage, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (cq *ChatQuery) loadChatMessage(ctx context.Context, query *MessageQuery, nodes []*Chat, init func(*Chat), assign func(*Chat, *Message)) error {
	...
	query.Where(predicate.Message(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(chat.ChatMessageColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
