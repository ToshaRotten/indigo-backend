...
type OrderQuery struct {
	...
	withUser        *UserQuery
	withAddresOrder *AddressQuery
	withOrder       *OrderDetailQuery
...
func (oq *OrderQuery) QueryOrderStatus() *OrderStatusQuery {
	query := (&OrderStatusClient{config: oq.config}).Query()
...
// QueryAddresOrder chains the current query on the "addres_order" edge.
func (oq *OrderQuery) QueryAddresOrder() *AddressQuery {
...
func (oq *OrderQuery) QueryOrder() *OrderDetailQuery {
...
func (oq *OrderQuery) Clone() *OrderQuery {
	...
	return &OrderQuery{
		...
		withAddresOrder: oq.withAddresOrder.Clone(),
		withOrder:       oq.withOrder.Clone(),
...
func (oq *OrderQuery) WithOrderStatus(opts ...func(*OrderStatusQuery)) *OrderQuery {
	query := (&OrderStatusClient{config: oq.config}).Query()
...
// WithAddresOrder tells the query-builder to eager-load the nodes that are connected to
// the "addres_order" edge. The optional arguments are used to configure the query builder of the edge.
func (oq *OrderQuery) WithAddresOrder(opts ...func(*AddressQuery)) *OrderQuery {
	query := (&AddressClient{config: oq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	oq.withAddresOrder = query
	return oq
}

// WithOrder tells the query-builder to eager-load the nodes that are connected to
// the "order" edge. The optional arguments are used to configure the query builder of the edge.
func (oq *OrderQuery) WithOrder(opts ...func(*OrderDetailQuery)) *OrderQuery {
	query := (&OrderDetailClient{config: oq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (oq *OrderQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Order, error) {
	...
	if query := oq.withOrderStatus; query != nil {
		if err := oq.loadOrderStatus(ctx, query, nodes, nil,
			func(n *Order, e *OrderStatus) { n.Edges.OrderStatus = e }); err != nil {
			return nil, err
		}
	}
	...
	if query := oq.withAddresOrder; query != nil {
		if err := oq.loadAddresOrder(ctx, query, nodes, nil,
			func(n *Order, e *Address) { n.Edges.AddresOrder = e }); err != nil {
			return nil, err
		}
	}
	if query := oq.withOrder; query != nil {
		if err := oq.loadOrder(ctx, query, nodes,
			func(n *Order) { n.Edges.Order = []*OrderDetail{} },
			func(n *Order, e *OrderDetail) { n.Edges.Order = append(n.Edges.Order, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (oq *OrderQuery) loadOrderStatus(ctx context.Context, query *OrderStatusQuery, nodes []*Order, init func(*Order), assign func(*Order, *OrderStatus)) error {
...
func (oq *OrderQuery) loadAddresOrder(ctx context.Context, query *AddressQuery, nodes []*Order, init func(*Order), assign func(*Order, *Address)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Order)
	for i := range nodes {
...
func (oq *OrderQuery) loadOrder(ctx context.Context, query *OrderDetailQuery, nodes []*Order, init func(*Order), assign func(*Order, *OrderDetail)) error {
	...
	query.Where(predicate.OrderDetail(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(order.OrderColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (oq *OrderQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := oq.ctx.Fields; len(fields) > 0 {
		...
		if oq.withOrderStatus != nil {
			_spec.Node.AddColumnOnce(order.FieldOrderStatusID)
		}
		if oq.withUser != nil {
			_spec.Node.AddColumnOnce(order.FieldUserID)
		}
		if oq.withAddresOrder != nil {
